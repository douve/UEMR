...
etl.process = function(data,id.var = 'idp',dat.var = 'dat',end.dat.var = 'dbaixa',
  ...
  n.cores = n.cores
  cl <- snow::makeSOCKcluster(n.cores, outfile='') # make cluster
  ...
  s = Sys.time()
  pETL = foreach(i = 1:niter, .packages = c("Kendall",'UEMR','data.table','anytime','dplyr'),
                 .options.snow = opts, 
                 .combine = "rbind.data.frame", .multicombine = F) %dopar% {
